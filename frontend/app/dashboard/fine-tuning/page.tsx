...
export default function FineTuningPage() {
  ...
  // KPI & chart data (placeholder — will be replaced by real API data later)
  const [kpiStats] = useState({ datasets: 0, jobs: 0, activeModel: 'qwen3.5:27b', lastEval: 'N/A' });
  const [lossData] = useState([
    ...
    <DashboardLayout title="Fine-Tuning" description="Entrainement de modeles specialises">
      <div className="h-full w-full space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard title="Datasets" value={kpiStats.datasets} icon={Database} />
          <KpiCard title="Jobs lances" value={kpiStats.jobs} icon={Play} />
          <KpiCard title="Modele actif" value={kpiStats.activeModel || 'Aucun'} icon={Brain} />
          <KpiCard title="Derniere eval" value={kpiStats.lastEval || 'N/A'} icon={CheckCircle} />
        </div>

        {/* Training Charts */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <ChartWrapper title="Courbe de loss" subtitle="Training vs Validation">
          ...
          <ChartWrapper title="Scores d'evaluation" subtitle="Avant / Apres fine-tuning">
            <ResponsiveContainer width="100%" height={250}>
