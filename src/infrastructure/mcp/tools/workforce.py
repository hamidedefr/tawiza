...
---
*Rapport genere par MPtoO Workforce - {datetime.now().strftime('%d/%m/%Y %H:%M')}*
"""
    return rapport_md
...
def register_workforce_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    async def mptoo_workforce_analyze(
        query: str,
...
---
*Rapport genere le {datetime.now().strftime("%d/%m/%Y a %H:%M")} par MPtoO Workforce*
"""
    ...
    @mcp.tool()
    async def mptoo_workforce_status(ctx: Context = None) -> str:
