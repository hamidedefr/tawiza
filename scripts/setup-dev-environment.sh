#!/bin/bash
# MPtoO v2.0.3 - Automated Development Environment Setup
...
# Banner
echo "================================================================================"
echo "  MPtoO v2.0.3 - Development Environment Setup"
...
setup_environment() {
    ...
    if [ ! -f .env ]; then
        if [ -f .env.example ]; then
            ...
            cat > .env << 'EOF'
# MPtoO v2.0.3 Environment Configuration
APP_ENV=development
DEBUG=true

# Database
DATABASE__URL=postgresql+asyncpg://mptoo:password@localhost:5432/mptoo
...
setup_services() {
    ...
    # Check PostgreSQL
    if docker-compose exec -T postgres pg_isready -U mptoo &> /dev/null; then
