...
class AgentOrchestrator:
    """
    Orchestrates requests between MPtoO agents and Ollama models.

    Routing logic:
    - Models starting with "mptoo-" -> Route to MPtoO agents
    - Other models -> Forward to Ollama

    Available MPtoO agents:
    - mptoo-analyst: Strategic analysis and territorial intelligence
    - mptoo-data: Sirene data collection
    - mptoo-geo: Mapping and geolocation
    - mptoo-veille: Market monitoring (BODACC, BOAMP)
    - mptoo-finance: Financial data analysis
    - mptoo-simulation: Scenario simulation
    - mptoo-prospection: B2B lead generation
    - mptoo-comparison: Territory comparison
    - mptoo-business-plan: Business plan generation
    """
    ...
    def __init__(self):
        ...
        logger.info(
            f"AgentOrchestrator initialized with {len(self.mptoo_agents)} MPtoO agents "
    ...
    def is_mptoo_agent(self, model: str) -> bool:
        """Check if model is a MPtoO agent."""
        return model.startswith("mptoo-")

    def get_agent_info(self, model: str) -> dict[str, str] | None:
        """Get agent information."""
        return self.mptoo_agents.get(model)
    ...
    async def chat_completion(
        ...
        """
        if self.is_mptoo_agent(request.model):
            return await self._mptoo_agent_completion(request)
        else:
    ...
    async def chat_completion_stream(
        ...
        """
        if self.is_mptoo_agent(request.model):
            async for chunk in self._mptoo_agent_completion_stream(request):
                yield chunk
        else:
            async for chunk in self._ollama_completion_stream(request):
                yield chunk

    async def _mptoo_agent_completion(
        self, request: ChatCompletionRequest
    ) -> ChatCompletionResponse:
        """Process MPtoO agent request."""
        agent_info = self.get_agent_info(request.model)
        if not agent_info:
            raise ValueError(f"Unknown MPtoO agent: {request.model}")
    ...
    async def _mptoo_agent_completion_stream(
        self, request: ChatCompletionRequest
    ) -> AsyncIterator[str]:
        """Process streaming MPtoO agent request."""
        agent_info = self.get_agent_info(request.model)
        if not agent_info:
            raise ValueError(f"Unknown MPtoO agent: {request.model}")
    ...
    def _build_agent_system_prompt(
        ...
        """Build system prompt for MPtoO agent."""
        agent_prompts = {
            "mptoo-analyst": """You are the MPtoO Analyst Agent, an expert in strategic analysis and territorial intelligence for French territories.
...
Always respond in French unless asked otherwise.""",
            "mptoo-data": """You are the MPtoO Data Agent, specialized in collecting and processing data from the Sirene database and other French administrative sources.
...
You provide accurate, structured data with proper source attribution.""",
            "mptoo-geo": """You are the MPtoO Geo Agent, specialized in mapping, geolocation, and spatial analysis of French territories.
...
You provide precise geographic insights with proper coordinates and references.""",
            "mptoo-veille": """You are the MPtoO Veille Agent, specialized in market monitoring using BODACC and BOAMP data sources.
...
You provide timely, relevant market intelligence.""",
            "mptoo-finance": """You are the MPtoO Finance Agent, specialized in financial analysis of companies and territories.
...
You provide thorough financial insights with proper calculations and sources.""",
            "mptoo-simulation": """You are the MPtoO Simulation Agent, specialized in scenario simulation and forecasting.
...
You provide data-driven simulations with clear assumptions and limitations.""",
            "mptoo-prospection": """You are the MPtoO Prospection Agent, specialized in B2B lead generation and prospection.
...
You provide targeted, actionable prospection insights.""",
            "mptoo-comparison": """You are the MPtoO Comparison Agent, specialized in territory comparison and benchmarking.
...
You provide objective, data-driven comparisons.""",
            "mptoo-business-plan": """You are the MPtoO Business Plan Agent, specialized in business plan generation and analysis.
...
You provide comprehensive, professional business plans.""",
    ...
    async def list_models(self) -> list[dict[str, Any]]:
        """List available models (MPtoO agents + Ollama models)."""
        models = []

        # Add MPtoO agents
        for agent_id, agent_info in self.mptoo_agents.items():
            models.append(
                {
                    "id": agent_id,
                    "object": "model",
