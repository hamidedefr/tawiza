"""Services dashboard commands for MPtoO CLI v2 pro.

Provides a unified dashboard with links to all MPtoO services including:
...
QUICK_LINKS = {
    "ML Platform": [
        ("Fine-tune model", "mptoo pro train-start <model> -d <data>"),
        ("Training status", "mptoo pro train-status"),
        ("List models", "mptoo pro model-list"),
        ("Pull model", "mptoo pro model-pull <model>"),
    ],
    "Annotations": [
        ("Create project", "mptoo annotate create -n <name>"),
        ("List projects", "mptoo annotate list"),
        ("Export annotations", "mptoo annotate export <id>"),
        ("Upload tasks", "mptoo annotate upload <id> -f <file>"),
    ],
    "Agents": [
        ("List agents", "mptoo pro agents"),
        ("Run Manus agent", "mptoo pro agent-run manus -t <task>"),
        ("Run S3 agent", "mptoo pro agent-run s3 -t <task>"),
        ("Debug agent", "mptoo pro agent-debug"),
    ],
    "Monitoring": [
        ("GPU monitor", "mptoo pro gpu-monitor"),
        ("System dashboard", "mptoo pro dashboard"),
        ("View logs", "mptoo pro logs-show"),
        ("Export metrics", "mptoo pro metrics-export <file>"),
    ],
}
...
def register(app: typer.Typer) -> None:
    ...
    def list_services(
        ...
        """Show all MPtoO services with status and links."""
        ...
        console.print("  [dim]Check health:[/] mptoo pro services --check")
    ...
    def unified_dashboard(
        ...
        def build_dashboard() -> Layout:
            ...
            # Header
            header_text = Text()
            header_text.append("MPtoO Dashboard", style="bold cyan")
    ...
    def open_service(
        ...
        if is_healthy:
            ...
            if service_id == "ollama":
                console.print("  [dim]Start with:[/] mptoo pro ollama-start")
            elif service_id == "api":
                console.print("  [dim]Start with:[/] mptoo system start")
