...
if command -v docker &>/dev/null; then
    ...
    # Existing MPtoO containers
    MPTOO_CONTAINERS=$(docker ps -a --filter "name=mptoo-" --filter "name=openmanus-" --format "{{.Names}}: {{.Status}}" 2>/dev/null || true)
    if [[ -n "$MPTOO_CONTAINERS" ]]; then
        echo -e "  ${CYAN}Existing MPtoO containers:${NC}"
        ...
        done <<< "$MPTOO_CONTAINERS"
    else
        ok "No existing MPtoO containers"
    fi
fi
