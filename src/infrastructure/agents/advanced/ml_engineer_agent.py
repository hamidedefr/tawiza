#!/usr/bin/env python3
"""
MLEngineerAgent - Agent spécialisé en ML engineering pour MPtoO-V2
...
class MLEngineerAgent:
    ...
    async def _save_model(self, model: Any, config: MLTrainingConfig) -> str:
        ...
        try:
            ...
            model_path = f"/tmp/mptoo_models/{config.task_id}_model.pkl"
            Path("/tmp/mptoo_models").mkdir(parents=True, exist_ok=True)
