...
def register_prospection_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    async def mptoo_prospect_export(
        leads_json: str,
        ...
        Args:
            leads_json: JSON des leads (resultat de mptoo_prospect)
        ...
        export_dir = Path.home() / ".mptoo" / "exports" / "prospects"
    ...
    @mcp.tool()
    async def mptoo_generate_message(
        entreprise: str,
        secteur: str,
        offre: str,
...
Message:"""
    ...
    @mcp.tool()
    async def mptoo_financial_health(
        siret: str,
        ctx: Context = None,
    ) -> str:
