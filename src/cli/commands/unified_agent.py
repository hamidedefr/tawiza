...
def execute(
    ...
    if result.status == TaskStatus.AWAITING_APPROVAL:
        console.print(Panel(
            ...
            f"Use [cyan]mptoo uaa approve {result.task_id}[/cyan] to approve\n"
            f"Use [cyan]mptoo uaa reject {result.task_id}[/cyan] to reject",
...
def pending():
    ...
    console.print(table)
    console.print("\nUse [cyan]mptoo uaa approve <task_id>[/cyan] or [cyan]mptoo uaa reject <task_id>[/cyan]")
