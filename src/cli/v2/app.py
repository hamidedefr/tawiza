...
app = typer.Typer(
    name="mptoo",
    help="MPtoO - AI Multi-Agent Platform",
    add_completion=True,
...
def main(
    ...
    """MPtoO - AI Multi-Agent Platform with GPU Acceleration."""
...
def status(
    ...
    [bold]Examples:[/]
      mptoo status          Quick status overview
      mptoo status -v       Detailed system info
    """
...
def chat(
    ...
    [bold]Examples:[/]
      mptoo chat                    Interactive chat session
      mptoo chat "Hello!"           Send a quick message
      mptoo chat -m llama3:8b       Use a specific model
      mptoo chat --no-interactive   Single response mode
    """
...
def run(
    ...
    [bold]Examples:[/]
      mptoo run                              Interactive agent selection
      mptoo run analyst -d data.csv          Analyze a dataset
      mptoo run browser -t "search Google"   Web automation
      mptoo run coder -t "Create a function" Generate code
      mptoo run ml -d train.csv              Train ML model
    """
...
def agent(
    ...
    [bold]Examples:[/]
      mptoo agent "What time is it in Tokyo?"
      mptoo agent "Analyze trends" -d data.csv
      mptoo agent "Scrape and summarize HN" -v
      mptoo agent --interactive  # Conversation mode
    """
...
def analyze(
    ...
    [bold]Examples:[/]
      ...
      mptoo analyze "startups IA Hauts-de-France" --depth full
      mptoo analyze "startups IA Lille" --use-agents -v
      mptoo analyze "startup IA Lille" --multi-source  # 8 sources + validation
      mptoo analyze "conseil IT Lyon" -i  # Interactive follow-ups
    """
...
def tajine(
    ...
    [bold]Examples:[/]
      mptoo tajine "Analyse tech sector in 34"
      mptoo tajine --territory 34 --sector tech -v
      mptoo tajine "Compare 75 vs 13" --real-api
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if models:
                ...
                console.print("  mptoo pro model-pull qwen3.5:27b")
        else:
            msg = MessageBox()
        ...
        console.print(msg.error(
            ...
            ["Start with: mptoo pro ollama-start"]
        ))
