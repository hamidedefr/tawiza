...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem('mptoo-theme');
    const savedSidebar = localStorage.getItem('mptoo-sidebar-collapsed');
    const savedCognitive = localStorage.getItem('mptoo-cognitive-level');
    const savedFormat = localStorage.getItem('mptoo-export-format');
    const savedStreaming = localStorage.getItem('mptoo-streaming');
    const savedFavorites = localStorage.getItem('mptoo-favorites');
    const savedOllamaUrl = localStorage.getItem('mptoo-ollama-url');
    // Note: defaultModel is now managed by the backend via useOllamaModels hook
    const savedOpenaiKey = localStorage.getItem('mptoo-openai-key');
    const savedAnthropicKey = localStorage.getItem('mptoo-anthropic-key');

    // Data Sources
    const savedSireneEnabled = localStorage.getItem('mptoo-sirene-enabled');
    const savedBodaccEnabled = localStorage.getItem('mptoo-bodacc-enabled');
    const savedBoampEnabled = localStorage.getItem('mptoo-boamp-enabled');
    const savedBanEnabled = localStorage.getItem('mptoo-ban-enabled');
    const savedInseeToken = localStorage.getItem('mptoo-insee-token');

    // Knowledge Graph
    const savedNeo4jUrl = localStorage.getItem('mptoo-neo4j-url');
    const savedNeo4jUser = localStorage.getItem('mptoo-neo4j-user');
    const savedNeo4jPassword = localStorage.getItem('mptoo-neo4j-password');
    const savedNeo4jEnabled = localStorage.getItem('mptoo-neo4j-enabled');

    // Crawler
    const savedCrawlerRateLimit = localStorage.getItem('mptoo-crawler-rate-limit');
    const savedCrawlerStealthMode = localStorage.getItem('mptoo-crawler-stealth');
    const savedCrawlerUserAgent = localStorage.getItem('mptoo-crawler-user-agent');
    const savedCrawlerMaxRetries = localStorage.getItem('mptoo-crawler-max-retries');

    // Monitoring
    const savedSentryDsn = localStorage.getItem('mptoo-sentry-dsn');
    const savedLoggingLevel = localStorage.getItem('mptoo-logging-level');
    const savedTelemetryEnabled = localStorage.getItem('mptoo-telemetry-enabled');

    // Agent
    const savedAgentAutonomy = localStorage.getItem('mptoo-agent-autonomy');
    const savedMaxDelegationDepth = localStorage.getItem('mptoo-max-delegation-depth');
    const savedConfirmCritical = localStorage.getItem('mptoo-confirm-critical');
    const savedBrowserActions = localStorage.getItem('mptoo-browser-actions');
  ...
  const handleSave = () => {
    // Basic settings
    localStorage.setItem('mptoo-theme', theme);
    localStorage.setItem('mptoo-sidebar-collapsed', String(sidebarCollapsed));
    localStorage.setItem('mptoo-cognitive-level', defaultCognitiveLevel);
    localStorage.setItem('mptoo-export-format', exportFormat);
    localStorage.setItem('mptoo-streaming', String(streamingEnabled));
    localStorage.setItem('mptoo-favorites', JSON.stringify(favorites));
    localStorage.setItem('mptoo-ollama-url', ollamaUrl);
    // Note: defaultModel is saved via the backend API, not localStorage
    localStorage.setItem('mptoo-openai-key', openaiKey);
    localStorage.setItem('mptoo-anthropic-key', anthropicKey);

    // Data Sources
    localStorage.setItem('mptoo-sirene-enabled', String(sireneEnabled));
    localStorage.setItem('mptoo-bodacc-enabled', String(bodaccEnabled));
    localStorage.setItem('mptoo-boamp-enabled', String(boampEnabled));
    localStorage.setItem('mptoo-ban-enabled', String(banEnabled));
    localStorage.setItem('mptoo-insee-token', inseeToken);

    // Knowledge Graph
    localStorage.setItem('mptoo-neo4j-url', neo4jUrl);
    localStorage.setItem('mptoo-neo4j-user', neo4jUser);
    localStorage.setItem('mptoo-neo4j-password', neo4jPassword);
    localStorage.setItem('mptoo-neo4j-enabled', String(neo4jEnabled));

    // Crawler
    localStorage.setItem('mptoo-crawler-rate-limit', String(crawlerRateLimit));
    localStorage.setItem('mptoo-crawler-stealth', String(crawlerStealthMode));
    localStorage.setItem('mptoo-crawler-user-agent', crawlerUserAgent);
    localStorage.setItem('mptoo-crawler-max-retries', String(crawlerMaxRetries));

    // Monitoring
    localStorage.setItem('mptoo-sentry-dsn', sentryDsn);
    localStorage.setItem('mptoo-logging-level', loggingLevel);
    localStorage.setItem('mptoo-telemetry-enabled', String(telemetryEnabled));

    // Agent
    localStorage.setItem('mptoo-agent-autonomy', agentAutonomy);
    localStorage.setItem('mptoo-max-delegation-depth', String(maxDelegationDepth));
    localStorage.setItem('mptoo-confirm-critical', String(confirmCriticalActions));
    localStorage.setItem('mptoo-browser-actions', String(browserActionsEnabled));
  ...
  const handleExportData = () => {
    ...
    a.href = url;
    a.download = 'mptoo-settings.json';
    a.click();
