...
PIPELINES_DIR = Path.home() / ".mptoo" / "pipelines"
...
def run_pipeline(
    ...
    Examples:
        # Run a pipeline
        mptoo pipelines run my-pipeline.yaml

        # Dry run (preview only)
        mptoo pipelines run my-pipeline.yaml --dry-run
    """
...
def list_pipelines():
    ...
    if not yaml_files:
        ...
        console.print("[dim]Or use: mptoo pipelines create <name>[/dim]")
        return
...
def create_pipeline(
    ...
    Examples:
        ...
        mptoo pipelines create my-pipeline

        # Create web scraping pipeline
        mptoo pipelines create scraper --template web-scrape
    """
    templates = {
        "basic": {
            "name": name,
    ...
    try:
        ...
        console.print(f"[dim]Run with: mptoo pipelines run {name}.yaml[/dim]")
...
def show_pipeline(
    ...
    Examples:
        mptoo pipelines show my-pipeline.yaml
    """
    pipeline_path = Path(pipeline_file)
