...
app = typer.Typer(help="Chat with the MPtoO AI assistant")
console = Console()

# History directory
HISTORY_DIR = Path.home() / ".mptoo" / "chat_history"
...
def start(
    ...
    Examples:
        ...
        mptoo chat start --model mistral:latest

        # Enable RAG for knowledge-enhanced responses
        mptoo chat start --rag

        # Resume an existing session
        mptoo chat start --session abc-123-def
    """
    ...
    console.print(Panel.fit(
        "[bold cyan]MPtoO AI Assistant[/bold cyan]\n\n"
        "Ask me anything about the MPtoO platform!\n"
...
async def _chat_loop(
    ...
    try:
        ...
        console.print(f"  2. Model '{model}' is available (mptoo models list)")
        return
    ...
    system_prompt = """You are MPtoO AI Assistant, an expert in the MPtoO platform.
You help users with:
    ...
    while True:
        try:
            ...
            if user_input.lower().strip() in ["exit", "quit", "bye", "q"]:
                ...
                console.print(f"[dim]Resume with: mptoo chat start --session {session_id}[/dim]\n")
                break
...
def history(
    ...
    Examples:
        ...
        mptoo chat history abc-123-def

        # View last 20 messages
        mptoo chat history abc-123-def --limit 20
    """
...
def sessions():
    ...
    if not saved_sessions:
        ...
        console.print("[dim]Start a new session with: mptoo chat start[/dim]")
        return
    ...
    console.print()
    console.print(table)
    console.print("\n[dim]Resume with: mptoo chat start --session <SESSION_ID>[/dim]")
...
def clear(
    ...
    Examples:
        ...
        mptoo chat clear abc-123-def

        # Force clear without confirmation
        mptoo chat clear abc-123-def --force
    """
...
def export(
    ...
    Examples:
        ...
        mptoo chat export abc-123-def

        # Export to specific file
        mptoo chat export abc-123-def --output my-chat.json

        # Export as markdown
        mptoo chat export abc-123-def --format markdown --output chat.md
    """
...
def suggest(
    ...
    Examples:
        mptoo chat suggest "show me all models"
        mptoo chat suggest "start training a model"
        mptoo chat suggest "check system health"
    """
    ...
    if suggestion:
        ...
        console.print(get_sunset_banner("[dim]Try being more specific or use 'mptoo chat start' for interactive help[/dim]"))
