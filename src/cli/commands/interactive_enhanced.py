#!/usr/bin/env python3
"""
Commandes interactives améliorées pour MPtoO-V2
...
def _get_path_validator() -> Optional["PathValidator"]:
    ...
    try:
        return PathValidator(
            allowed_directories=[
                Path("/root/MPtoO-V2/data"),
                Path("/root/MPtoO-V2/datasets"),
                Path("/root/MPtoO-V2/output"),
                Path("/root/MPtoO-V2/configs"),
                Path.home() / "data",
            ],
...
@app.command()
def wizard():
    """🧙 Assistant interactif complet pour MPtoO-V2"""

    console.print(Panel.fit(
        "🧙 [bold #FF6B35]MPtoO-V2 Interactive Wizard[/bold #FF6B35]\n"
...
def _training_wizard():
    ...
    if questionary.confirm(
        ...
        console.print("[bold #B0B0B0]Utilisez 'mptoo finetune status' pour suivre la progression[/bold #B0B0B0]")
