#!/usr/bin/env python3
"""Collecteur unifié MPtoO V2 — Toutes sources vers table `signals`.
...
async def store_signals(signals: list[Signal]) -> int:
    ...
    db_url = os.getenv("DATABASE_URL", "postgresql://mptoo:changeme@localhost:5433/mptoo")
...
async def run_full_collect(
    ...
    logger.info(f"🚀 COLLECTE MPtoO V2")
    ...
    # Résumé
    logger.info(f"\n{'='*60}")
    logger.info(f"📊 RÉSUMÉ COLLECTE MPtoO V2")
    logger.info(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Collecteur unifié MPtoO V2")
