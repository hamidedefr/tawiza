"""Training commands for MPtoO CLI v2 pro."""
...
# Jobs storage
JOBS_FILE = Path.home() / ".mptoo" / "jobs.json"
...
def register(app: typer.Typer) -> None:
    ...
    def train_start(
        ...
        console.print(msg.success(
            ...
            f"Monitor with: mptoo pro train-status {job_id}"
        ))

        # Note: Actual training would be done asynchronously
        console.print()
        console.print("  [dim]Note: Training runs in background.[/]")
        console.print("  [dim]Check status with: mptoo pro train-status[/]")
    ...
    def train_status(
        ...
        if not jobs:
            ...
            console.print("  [dim]Start one with: mptoo pro train-start[/]")
            console.print(footer(40))
            return
