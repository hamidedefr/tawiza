...
Provides CLI interface for Docker operations integrated with MPtoO.
"""
...
def run_container(
    ...
    Examples:
        # Run nginx
        mptoo docker run nginx --name web --port 8080:80

        # Run with GPU
        mptoo docker run ollama/ollama --gpu --name ollama

        # Run with environment
        mptoo docker run postgres --env POSTGRES_PASSWORD=secret
    """
    if not _docker_available():
        console.print("[red]Docker not available or not running[/red]")
        return
...
def stop_container(
    ...
    Examples:
        mptoo docker stop web
        mptoo docker stop abc123 --force
    """
    if not _docker_available():
        console.print("[red]Docker not available or not running[/red]")
        return
...
def container_logs(
    ...
    Examples:
        mptoo docker logs web
        mptoo docker logs web --follow
        mptoo docker logs web --tail 100
    """
    if not _docker_available():
        console.print("[red]Docker not available or not running[/red]")
        return
...
def exec_container(
    ...
    Examples:
        mptoo docker exec web bash
        mptoo docker exec web "ls -la"
    """
    if not _docker_available():
        console.print("[red]Docker not available or not running[/red]")
        return
...
def pull_image(
    ...
    Examples:
        mptoo docker pull nginx
        mptoo docker pull ollama/ollama:latest
    """
    if not _docker_available():
        console.print("[red]Docker not available or not running[/red]")
        return
...
def container_stats(
    ...
    Examples:
        mptoo docker stats
        mptoo docker stats web --no-stream
    """
    if not _docker_available():
        console.print("[red]Docker not available or not running[/red]")
        return
