...
def agent_command(
    ...
    Examples:
        ...
        mptoo agent "Analyze this CSV and create a summary" -d data.csv
        mptoo agent "Scrape HN and find AI trends" --verbose
        mptoo agent --interactive  # Start conversation mode
    """
    if interactive:
...
async def _run_agent(
    ...
    if dry_run:
        console.print(header("mptoo agent", 40))
        console.print()
    ...
    try:
        ...
        if not healthy:
            console.print(msg.error(
                ...
                ["Start with: ollama serve", "Or: mptoo pro ollama-start"]
            ))
            return
...
async def _run_interactive(
    ...
    history_file = Path.home() / ".mptoo" / "agent_history"
