...
def run_task(
    ...
    Examples:
        ...
        mptoo agent run "Go to news.ycombinator.com and get the top 5 articles"

        # Start from specific URL
        mptoo agent run "Find the pricing page" --url https://example.com

        # Dry-run to test the plan without browser
        mptoo agent run "Fill contact form" --url https://example.com/contact --dry-run

        # Auto-approve for scripting
        mptoo agent run "Extract product prices" --url https://shop.com --auto

        # Use different model
        mptoo agent run "Complex task" --model qwen3.5:27b
    """
    # Welcome
    if dry_run:
...
def show_plan(
    ...
    Examples:
        mptoo agent plan "Search for flights from Paris to Tokyo"
        mptoo agent plan "Book hotel" --url https://booking.com --output plan.json
    """
    if not json_output:
...
def show_status(
    ...
    Examples:
        mptoo agent status                    # Recent tasks
        mptoo agent status plan-abc123        # Specific task
        mptoo agent status --all --limit 20   # More history
    """
    try:
...
def cancel_task(
    ...
    Example:
        mptoo agent cancel plan-abc123
    """
    try:
...
def resume_task(
    ...
    Example:
        mptoo agent resume plan-abc123
        mptoo agent resume plan-abc123 --dry-run
    """
