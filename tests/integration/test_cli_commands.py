...
class TestSystemCommands:
    ...
    def test_help_command(self):
        """Test that mptoo --help works."""
        result = subprocess.run(["mptoo", "--help"], capture_output=True, text=True, timeout=10)
        assert result.returncode == 0
        assert "Usage:" in result.stdout
        assert "system" in result.stdout

    def test_version_command(self):
        """Test that mptoo version works."""
        result = subprocess.run(["mptoo", "version"], capture_output=True, text=True, timeout=10)
        # Should show version info
        assert "MPtoO" in result.stdout or "version" in result.stdout.lower()

    def test_system_help(self):
        """Test system --help command."""
        result = subprocess.run(
            ["mptoo", "system", "--help"], capture_output=True, text=True, timeout=10
        )
        assert result.returncode == 0
    ...
    def test_system_gpu(self):
        ...
        result = subprocess.run(
            ["mptoo", "system", "gpu"], capture_output=True, text=True, timeout=10
        )
    ...
    def test_system_health_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "system", "health"], capture_output=True, text=True, timeout=15
        )
    ...
    def test_system_status_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "system", "status"], capture_output=True, text=True, timeout=15
        )
    ...
    def test_system_diagnose_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "system", "diagnose"], capture_output=True, text=True, timeout=20
        )
    ...
    def test_system_services_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "system", "services"], capture_output=True, text=True, timeout=20
        )
...
class TestFineTuneCommands:
    ...
    def test_finetune_help(self):
        ...
        result = subprocess.run(
            ["mptoo", "finetune", "--help"], capture_output=True, text=True, timeout=10
        )
        assert result.returncode == 0
    ...
    def test_finetune_start_help(self):
        ...
        result = subprocess.run(
            ["mptoo", "finetune", "start", "--help"], capture_output=True, text=True, timeout=10
        )
        assert result.returncode == 0
    ...
    def test_finetune_list_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "finetune", "list"], capture_output=True, text=True, timeout=15
        )
    ...
    def test_finetune_models_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "finetune", "models"], capture_output=True, text=True, timeout=15
        )
...
class TestAnnotateCommands:
    ...
    def test_annotate_help(self):
        ...
        result = subprocess.run(
            ["mptoo", "annotate", "--help"], capture_output=True, text=True, timeout=10
        )
        assert result.returncode == 0
    ...
    def test_annotate_create_help(self):
        ...
        result = subprocess.run(
            ["mptoo", "annotate", "create", "--help"], capture_output=True, text=True, timeout=10
        )
        assert result.returncode == 0
        assert "--name" in result.stdout or "-n" in result.stdout
    ...
    def test_annotate_list_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "annotate", "list"], capture_output=True, text=True, timeout=15
        )
...
class TestModelsCommands:
    ...
    def test_models_help(self):
        ...
        result = subprocess.run(
            ["mptoo", "models", "--help"], capture_output=True, text=True, timeout=10
        )
        assert result.returncode == 0
        assert "list" in result.stdout
    ...
    def test_models_list_with_api(self):
        ...
        result = subprocess.run(
            ["mptoo", "models", "list"], capture_output=True, text=True, timeout=15
        )
...
class TestDataCommands:
    ...
    def test_data_help(self):
        ...
        result = subprocess.run(
            ["mptoo", "data", "--help"], capture_output=True, text=True, timeout=10
        )
        assert result.returncode == 0
        assert "list" in result.stdout
