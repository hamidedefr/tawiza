...
if command -v docker &>/dev/null && docker info &>/dev/null 2>&1; then
    CONTAINERS=$(docker ps -a --filter "name=mptoo-" --filter "name=openmanus-" --format "{{.Names}}|{{.Status}}|{{.State}}" 2>/dev/null || true)
    ...
    if [[ -n "$CONTAINERS" ]]; then
        ...
        echo -e "  ${YELLOW}No MPtoO/OpenManus containers found${NC}"
    fi
else
...
if [[ "$VERBOSE" == "--verbose" || "$VERBOSE" == "-v" ]]; then
    ...
    if command -v psql &>/dev/null; then
        if psql "postgresql://mptoo:$(grep DATABASE_PASSWORD /root/MPtoO-V2/.env 2>/dev/null | cut -d= -f2 | head -1)@localhost:5433/mptoo" -c "SELECT 1;" &>/dev/null 2>&1; then
