...
class CredentialManager:
    ...
    def __init__(
        ...
        Args:
            storage_path: Path to credentials file (default: ~/.mptoo/credentials.enc)
        ...
        """
        self.storage_path = storage_path or Path.home() / ".mptoo" / "credentials.enc"
    ...
    def _initialize_cipher(self, custom_key: bytes | None = None) -> Fernet:
        ...
        if self.use_keyring:
            try:
                ...
                key_str = keyring.get_password("mptoo", "credential_encryption_key")

                if not key_str:
                    # Generate new key and store in keyring
                    key = Fernet.generate_key()
                    keyring.set_password("mptoo", "credential_encryption_key", key.decode())
    ...
    def _get_fallback_cipher(self) -> Fernet:
        ...
        key_env = os.environ.get("MPTOO_CREDENTIAL_KEY")
        if key_env:
