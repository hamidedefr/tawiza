...
def _parse_args():
    parser = argparse.ArgumentParser(description="MPtoO MCP Server")
...
@mcp.resource("mptoo://sources")
def get_sources() -> str:
    """Liste des sources de données disponibles."""
    return """# Sources de données MPtoO
...
@mcp.resource("mptoo://agents")
def get_agents() -> str:
    """Description des agents de débat."""
    return """# Agents de Débat MPtoO
...
## Outil MCP: mptoo_workforce_analyze

```
mptoo_workforce_analyze(
...
def get_alerts_resource() -> str:
    ...
    return f"""# Alertes de Veille MPtoO
...
## Outils Disponibles
- `mptoo_veille_scan` - Scan avec filtrage LLM
- `mptoo_veille_digest` - Digest periodique
- `mptoo_veille_configure` - Configuration watchlist
- `mptoo_veille_alert_detail` - Detail d'une alerte
"""


@mcp.resource("mptoo://veille")
def get_veille_resource() -> str:
    """Documentation de la veille automatisee."""
    return """# Veille Automatisee MPtoO
...
```
mptoo_veille_digest(period: "today"|"week"|"month")
```

### mptoo_veille_configure
...
def run_server():
    ...
    if _args.sse:
        ...
        print(f"🚀 Starting MPtoO MCP Server (SSE) on http://{_args.host}:{_args.port}")
