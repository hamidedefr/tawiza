#!/bin/bash
# MPtoO CLI bash completion
# Install: source /path/to/mptoo.bash
# Or copy to: /etc/bash_completion.d/mptoo
...
_mptoo_completion() {
    ...
    case "${prev}" in
        ...
        train-status|train-stop|train-delete)
            # Complete job IDs
            local jobs=$(cat ~/.mptoo/jobs.json 2>/dev/null | grep -oP '"\K[0-9_]+(?=":)' || echo "")
...
# Register completion
complete -F _mptoo_completion mptoo

# Also support the python module invocation
complete -F _mptoo_completion "python -m src.cli.v2.app"
