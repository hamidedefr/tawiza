...
class MetricsScreen(Container):
    ...
    async def _load_task_stats(self) -> None:
        ...
        try:
            ...
            completed = int(r.get("mptoo:tasks:completed") or 0)
            running = int(r.get("mptoo:tasks:running") or 0)
            failed = int(r.get("mptoo:tasks:failed") or 0)
            tokens = int(r.get("mptoo:tokens:total") or 0)
    ...
    def action_export_csv(self) -> None:
        ...
        try:
            export_path = Path("/root/MPtoO-V2/logs/metrics_export.csv")
