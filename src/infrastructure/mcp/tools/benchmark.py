...
Benchmark system for MPtoO territorial intelligence agents.
...
def register_benchmark_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    async def mptoo_benchmark_run(
        tests: list[str] | None = None,
        quick_mode: bool = True,
        ctx: Context = None,
    ) -> str:
        """Execute une suite de tests de benchmark sur les outils MPtoO.
        ...
        report_md = f"""# Rapport de Benchmark MPtoO
...
"""
        ...
        report_md += f"""
---
*Benchmark MPtoO - {len(suite.results)} tests*
"""
    ...
    @mcp.tool()
    async def mptoo_benchmark_list(ctx: Context = None) -> str:
        ...
        return json.dumps({
            ...
            "usage": "mptoo_benchmark_run(tests=['sirene_basic', 'geocoding'], quick_mode=True)",
        }, ensure_ascii=False, indent=2)

    @mcp.tool()
    async def mptoo_benchmark_tool(
        tool_name: str,
        iterations: int = 3,
        ctx: Context = None,
    ) -> str:
    ...
    @mcp.tool()
    async def mptoo_benchmark_compare(
        tools: list[str],
        ctx: Context = None,
    ) -> str:
        ...
        for tool_name in tools:
            result = await mptoo_benchmark_tool(
                tool_name=tool_name,
                iterations=2,
                ctx=ctx,
            )
