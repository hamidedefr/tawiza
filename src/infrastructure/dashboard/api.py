"""Unified FastAPI backend for MPtoO Dashboard.
...
app = FastAPI(
    title="MPtoO Dashboard API",
    description="REST API for MPtoO Dashboard - Market Analysis Platform",
    version="1.0.0",
)
...
def get_db() -> DashboardDB:
    ...
    global db
    if db is None:
        db_path = Path.home() / ".mptoo" / "dashboard.db"
...
async def create_analysis(request: AnalysisCreate):
    """Create a new analysis (triggers MCP mptoo_analyze)."""
...
async def export_analysis(request: ExportRequest):
    ...
    export_dir = Path.home() / ".mptoo" / "exports"
    ...
    if request.format == "json":
        ...
        try:
            ...
            <head>
                <meta charset="utf-8">
                <title>MPtoO Analysis Report</title>
                <style>
...
def run_api_server(host: str = "0.0.0.0", port: int = 3001):
    """Run the API server."""
    import uvicorn
    print(f"Starting MPtoO Dashboard API on http://{host}:{port}")
