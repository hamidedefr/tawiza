...
def config(
    ...
    Examples:
        ...
        mptoo captcha config --provider 2captcha

        # Configure with API key inline
        mptoo captcha config --provider 2captcha --api-key YOUR_KEY
    """
    ...
    env_var_name = f"MPTOO_CAPTCHA_{provider.upper().replace('-', '_')}_KEY"
    os.environ[env_var_name] = api_key
    os.environ["MPTOO_CAPTCHA_PROVIDER"] = provider
    os.environ["MPTOO_CAPTCHA_TIMEOUT"] = str(timeout)
...
[yellow]Note:[/yellow] Add these to your .env file for persistence:
  ...
  MPTOO_CAPTCHA_PROVIDER={provider}
  MPTOO_CAPTCHA_TIMEOUT={timeout}""",
...
def test(
    ...
    Examples:
        ...
        mptoo captcha test --provider 2captcha --api-key YOUR_KEY
    """
    import asyncio
    ...
    # Get configuration
    if not provider:
        provider = os.environ.get("MPTOO_CAPTCHA_PROVIDER", "2captcha")
    ...
    if not api_key:
        console.print(
            "[red]❌ No API key configured. Run 'mptoo captcha config' first.[/red]"
        )
        raise typer.Exit(1)

    timeout = int(os.environ.get("MPTOO_CAPTCHA_TIMEOUT", "120"))
...
def status():
    ...
    provider = os.environ.get("MPTOO_CAPTCHA_PROVIDER")
    timeout = os.environ.get("MPTOO_CAPTCHA_TIMEOUT")
...
[bold]Status:[/bold] ⚠️ Not ready (run 'mptoo captcha config')"""
    else:
        status_text = """[bold]Status:[/bold] ❌ Not configured

Run 'mptoo captcha config' to set up CAPTCHA solving."""
