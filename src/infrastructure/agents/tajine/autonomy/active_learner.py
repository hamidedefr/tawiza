...
class ActiveLearner:
    ...
    async def _run_collection(
        ...
        # Build command
        script = "/data/projects/mptoo-core/src/scripts/collect_all_v2.py"
        cmd = ["python3", script]
        if department:
    ...
    async def _run_detection(self, department: str | None = None) -> str:
        ...
        script = "/data/projects/mptoo-core/src/scripts/detect_microsignals_v2.py"
        cmd = ["python3", script]
        if department:
            cmd.extend(["--dept", department])
